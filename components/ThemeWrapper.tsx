...
export const THEME_STYLES: Record<ThemeType, ThemeDefinition> = {
  ...
  [ThemeType.BaseSoft]: {
    ...
    cardBase: "rounded-[3rem] p-4 flex items-center justify-between transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] shadow-sm border",
    cardColors: "bg-white/80 backdrop-blur-sm border-[#bfdbfe] text-[#1e40af]",
    cardHover: "hover:rounded-xl hover:bg-white hover:shadow-[0_10px_25px_rgba(59,130,246,0.2)] hover:border-[#60a5fa] hover:-translate-y-1 hover:scale-[1.01]",
    featuredCardOverlay: "from-blue-900/80 via-blue-600/30 to-transparent",
    actionIcon: "MoveUpRight",
  },
  [ThemeType.BaseDev]: {
    label: "Base Dev",
    bgClass: "bg-[#0f172a]",
  ...
  [ThemeType.BlackGem]: {
    ...
    cardHover: "hover:bg-[#222] hover:border-t-zinc-500 hover:shadow-2xl hover:scale-[1.01]",
  ...
  [ThemeType.BlackGemV2]: {
    ...
    cardHover: "hover:bg-white/10 hover:border-white/30 hover:shadow-[0_0_20_rgba(255,255,255,0.1)] hover:rounded-2xl hover:scale-[1.01]",
  ...
  [ThemeType.SunsetVibe]: {
    ...
    cardBase: "rounded-[3rem] p-4 flex items-center justify-between transition-all duration-300 shadow-lg backdrop-blur-md border",
    cardColors: "bg-white/5 border-white/5 text-white",
    cardHover: "hover:rounded-xl hover:bg-white/10 hover:shadow-rose-900/40 hover:scale-[1.02]",
  ...
  [ThemeType.UrbanJungle]: {
    ...
    cardHover: "hover:bg-black hover:border-purple-500 hover:translate-x-2",
  ...
  [ThemeType.MinimalistGrey]: {
    ...
    cardHover: "hover:translate-x-2 hover:border-gray-400 hover:text-black",
    featuredCardOverlay: "from-black/60 via-black/20 to-transparent",
    actionIcon: "ChevronRight",
  },
  [ThemeType.VibrantPop]: {
    ...
    cardColors: "bg-[#22d3ee] border-white text-black",
  ...
  [ThemeType.MidnightBloom]: {
    ...
    font: "font-serif",
    avatarClass: "rounded-tl-3xl rounded-br-3xl border-2 border-white shadow-2xl",
    socialClass: "bg-black/60 hover:bg-fuchsia-900/80 text-white hover:text-white rounded-xl backdrop-blur-md border border-white/20",
    cardBase: "rounded-tl-3xl rounded-br-3xl p-4 flex items-center justify-between transition-all duration-500 shadow-xl backdrop-blur-xl border",
    cardColors: "bg-black/75 border-white/20 text-white",
    cardHover: "hover:bg-black/85 hover:border-white/40 hover:shadow-[0_0_30px_rgba(0,0,0,0.4)]",
    featuredCardOverlay: "from-black via-black/60 to-transparent",
    actionIcon: "SquareArrowOutUpRight",
  },
  ...
  [ThemeType.CoffeeHouse]: {
    ...
    cardHover: "hover:bg-[#5d4037] hover:translate-x-2",
  ...
  [ThemeType.BubbleGum]: {
    ...
    cardBase: "rounded-[3rem] p-4 flex items-center justify-between transition-all duration-300 shadow-lg border-[3px]",
    cardColors: "bg-pink-400 border-white text-white",
    cardHover: "hover:rounded-xl hover:-translate-y-1 hover:bg-pink-300 hover:shadow-xl",
  ...
  [ThemeType.Newspaper]: {
    ...
    cardHover: "hover:bg-[#e6e6e0] hover:translate-x-2",
    featuredCardOverlay: "from-black/80 via-black/40 to-transparent",
    actionIcon: "ArrowRight",
  },
  [ThemeType.MintFresh]: {
    ...
    cardBase: "rounded-[3rem] p-4 flex items-center justify-between transition-all duration-300 shadow-sm border",
    cardColors: "bg-white border-[#bbf7d0] text-[#15803d]",
    cardHover: "hover:rounded-xl hover:shadow-md hover:border-[#86efac] hover:-translate-y-0.5",
...
export const ThemeWrapper: React.FC<ThemeWrapperProps> = ({ theme, children, className }) => {
  ...
  // Custom Darkening for themes like Midnight Bloom to ensure readability
  const needsExtraDarkness = theme?.type === ThemeType.MidnightBloom;

  return (
    <div className={`w-full relative overflow-x-hidden ${definition.textClass} ${definition.font} ${className || 'min-h-screen'}`} style={{...(theme?.customTextColor ? { color: theme.customTextColor } : {})}}>
        <div className={`fixed top-0 left-0 -z-10 w-full h-[100lvh] transition-colors duration-500 ease-in-out ${cleanBgClass}`} style={{...(theme?.customBackground ? { background: theme.customBackground } : {}), backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundAttachment: 'scroll'}} />
        {isImageBased && (<div className={`fixed inset-0 -z-10 w-full h-full ${needsExtraDarkness ? 'bg-black/55' : 'bg-black/15'} backdrop-blur-[1px]`} />)}
        <div className="relative z-0 w-full min-h-[inherit]">{children}</div>
    </div>
  );
};
...
export const getCardClasses = (theme: ProfileTheme, featured: boolean = false): string => {
  ...
  if (featured) {
      ...
      let base = styles.cardBase.replace(/\bp-\d+\s*/g, '').replace(/\bpx-\d+\s*/g, '').replace(/\bpy-\d+\s*/g, '').replace(/\bflex\b/g, '').replace(/\bitems-center\b/g, '').replace(/\bjustify-between\b/g, '');
      if (isListStyle) base = base.replace('rounded-none', 'rounded-xl').replace('border-b', 'border');
