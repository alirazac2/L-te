...
const ProfileView: React.FC<ProfileViewProps> = ({ initialData, disableNavigation, isPreview }) => {
    ...
    const LIGHT_THEMES = [
        ThemeType.Base, ThemeType.BaseSoft, ThemeType.CleanWhite, ThemeType.MinimalistGrey,
        ThemeType.SoftPastel, ThemeType.Newspaper, ThemeType.MintFresh, ThemeType.SketchyNote,
        ThemeType.NeoBrutalism, ThemeType.VintagePaper, ThemeType.NordicFrost, ThemeType.IslandParadise
    ];
            ...
            <ThemeWrapper theme={profile.theme} className={disableNavigation ? 'min-h-full' : 'min-h-screen'}>
                <div className="w-full flex flex-col items-center min-h-[inherit]">
                    <div className="w-full max-w-2xl px-5 py-12 md:py-20 flex flex-col items-center animate-fade-in relative z-10 flex-1">
                        ...
                        {/* Profile Menu */}
                        <div className="absolute top-4 left-4 md:left-0 md:top-8 z-30">
                            <ProfileMenu
                        ...
                        <div className="absolute top-4 right-4 md:right-0 md:top-8 z-20">
                            <button
                                ...
                                className={`
                                    ...
                                    ${isLightTheme ? 'bg-black/5 hover:bg-black/10 text-black' : 'bg-white/10 hover:bg-white/20 text-white'}
                                `}
                            >
                        ...
                        <header className="flex flex-col items-center mb-10 w-full">
                            <div className="relative mb-6 group cursor-pointer">
                                <AvatarWithSkeleton
                                    ...
                                    className={`w-28 h-28 md:w-32 md:h-32 relative z-10 ${themeStyles.avatarClass}`}
                                />
                            </div>
                                ...
                                <div className="flex flex-wrap justify-center gap-3 mt-6">
                                    {profile.socials.map((social, idx) => (
                                        <a
                                            ...
                                            className={`group flex items-center justify-center w-10 h-10 transition-all duration-300 transform hover:-translate-y-1 ${themeStyles.socialClass}`}
                        ...
                        <main className="w-full max-w-[600px] space-y-4 z-10 pb-10">
                            {profile.links.map((link, idx) => (
                                <LinkCard
                                    ...
                                    theme={profile.theme}
                                    index={idx}
                                    disableNavigation={disableNavigation || isPreview}
                                    onPreviewClick={handlePreviewAction}
                                />
                            ))}
                                            ...
                                            >
                                                {section.thumbnail ? (
                                                    <div className={`w-full aspect-[2/1] relative overflow-hidden shadow-2xl group cursor-pointer ring-1 ring-black/5 ${themeStyles.cardBase.split('p-4')[0]} p-0 ${themeStyles.cardColors}`}>
                                                        ...
                                                        />
                                                        <div className={`absolute inset-0 bg-gradient-to-t ${themeStyles.featuredCardOverlay} pointer-events-none`} />
                                                    ...
                                                    <div className={`group w-full ${themeStyles.cardBase} ${themeStyles.cardColors} ${themeStyles.cardHover}`}>
                                                        <div className="flex items-center gap-4">
                                                            <div className={`w-12 h-12 flex items-center justify-center shrink-0 opacity-80 ${themeStyles.avatarClass.includes('rounded-full') ? 'rounded-full' : 'rounded-lg'} border border-current`}>
                                                                {section.icon ? getGenericIcon(section.icon, "w-6 h-6") : <Layers className="w-6 h-6" />}
                                                            </div>
                                                            <div className="text-left flex-1">
                                                                <h3 className="font-bold text-lg">{section.title}</h3>
                                                                {section.description && <p className="text-xs opacity-70">{section.description}</p>}
                                                            </div>
                                                        </div>
...
}> = ({ link, theme, index, disableNavigation, onPreviewClick }) => {
            ...
            {link.featured ? (
                <div className="w-full aspect-[2/1] relative bg-gray-100">
                    <ImageWithSkeleton
                        ...
                        fallback={
                            <div className="opacity-50 flex items-center justify-center w-full h-full text-gray-400">
                                {getGenericIcon(link.icon, "w-12 h-12")}
                            </div>
                        }
                    />
                    <div className={`absolute inset-0 bg-gradient-to-t ${themeStyles.featuredCardOverlay} pointer-events-none`} />
                    <div className="absolute bottom-0 left-0 p-5 w-full z-10 text-white pointer-events-none">
                ...
                <>
                    <div className="flex items-center gap-4 min-w-0">
                        <ImageWithSkeleton
                            src={link.thumbnail}
                            className={`w-10 h-10 shrink-0 opacity-80 overflow-hidden ${thumbShape}`}
                            fallback={getGenericIcon(link.icon)}
                        />
...
const ImageWithSkeleton: React.FC<{
...
}> = ({ src, alt, className, imgClassName, fallback }) => {
    ...
    if (!src || error) {
        return (
            <div className={`flex items-center justify-center bg-black/5 ${className}`}>
                {fallback}
            </div>
        );
    }
    ...
    return (
        <div className={`relative overflow-hidden bg-black/5 ${className}`}>
            ...
            <img
                ...
                src={src}
                alt={alt || ''}
                className={`w-full h-full object-cover object-center transition-opacity duration-700 ease-in-out ${loaded ? 'opacity-100' : 'opacity-0'} ${imgClassName}`}
...
const AvatarWithSkeleton: React.FC<{
    src: string;
    alt: string;
    fallbackChar: string;
    className?: string
}> = ({ src, alt, fallbackChar, className }) => {
    const [loaded, setLoaded] = useState(false);
    ...
    return (
        <div className={`overflow-hidden bg-gray-200 relative ${className}`}>
            {src && !loaded && !err && <ShimmerOverlay />}
            {src && !err ? (
                <img
                    key={src}
                    src={src}
                    alt={alt}
                    className={`w-full h-full object-cover object-center transition-opacity duration-700 ease-in-out ${loaded ? 'opacity-100' : 'opacity-0'} relative z-10`}
                    onLoad={() => setLoaded(true)}
                    onError={() => setErr(true)}
                />
            ) : (
                <div className="w-full h-full flex items-center justify-center bg-gray-300 text-gray-600 font-bold text-3xl relative z-10">
...
const ProfileSkeleton = () => (
    <div className="min-h-screen bg-white flex justify-center">
        <div className="w-full max-w-2xl px-5 py-12 md:py-20 flex flex-col items-center">
            <div className="relative w-32 h-32 mb-8 rounded-full overflow-hidden shadow-sm">
                <ShimmerOverlay />
            </div>
            <div className="w-full flex flex-col items-center space-y-4 mb-10">
                <div className="h-8 w-48 rounded-xl relative overflow-hidden">
                    <ShimmerOverlay />
                </div>
                <div className="h-4 w-64 rounded-lg relative overflow-hidden">
                    <ShimmerOverlay />
                </div>
            </div>
            <div className="flex gap-4 mb-12">
                {[1, 2, 3, 4].map(i => (
                    <div key={i} className="w-10 h-10 rounded-full relative overflow-hidden">
                        <ShimmerOverlay />
                    </div>
                ))}
            </div>
            <div className="w-full max-w-[600px] space-y-4">
                {[1, 2, 3].map(i => (
                    <div key={i} className="w-full h-20 rounded-2xl relative overflow-hidden shadow-sm">
                        <ShimmerOverlay />
                    </div>
                ))}
                <div className="w-full aspect-[2/1] rounded-2xl relative overflow-hidden mt-2 shadow-sm">
                    <ShimmerOverlay />
                </div>
            </div>
        </div>
...
interface ProjectsDrawerProps {
    ...
    themeStyles: any;
    isPreview?: boolean;
    handlePreviewAction: (e: React.MouseEvent) => boolean;
}

const ProjectsDrawer: React.FC<ProjectsDrawerProps> = ({
    isOpen, onClose, section, themeStyles, isPreview, handlePreviewAction
}) => {
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex flex-col justify-end transition-all duration-500 font-sans ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
            ...
            />
            <div
                className={`relative w-full h-[92%] max-h-[92%] ${desktopClasses} rounded-t-[2.5rem] overflow-hidden flex flex-col shadow-[0_-10px_40px_rgba(0,0,0,0.3)] transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] transform ${isOpen ? 'translate-y-0' : 'translate-y-[120%]'} ${themeStyles.bgClass} ${themeStyles.textClass}`}
            >
                <div className="w-full flex justify-center pt-3 pb-1 cursor-pointer z-10 relative" onClick={onClose}>
                    <div className="w-12 h-1.5 bg-current opacity-20 rounded-full"></div>
                </div>
                <div className="px-6 py-4 flex items-center justify-between shrink-0 border-b border-gray-200/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs opacity-60">{projects.length} Items</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-full transition-colors hover:bg-white/10 opacity-70 hover:opacity-100">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                    ...
                    {projects.map((project, idx) => {
                        ...
                        return (
                            <div key={idx} className={`relative overflow-hidden ${cardRadius} border border-white/5 ${hasImage ? 'aspect-video' : `p-6 ${themeStyles.cardColors}`}`}>
                                {hasImage ? (
                                    <>
                                        <ImageWithSkeleton src={project.thumbnail} className="absolute inset-0 w-full h-full" imgClassName="object-cover" />
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent pointer-events-none" />
                                        <div className="absolute inset-0 flex flex-col justify-end p-5 text-white">
                                            {hasTags && (
                                                <div className="absolute top-4 left-4 flex flex-wrap gap-1.5 pointer-events-none">
                                                    {project.tags?.map((tag, tIdx) => (
                                                        <span key={tIdx} className="px-2 py-0.5 rounded bg-black/40 backdrop-blur-md border border-white/10 text-[9px] font-bold uppercase tracking-wider text-white/90">
                                                            {tag}
                                                        </span>
                                                    ))}
                                                </div>
                                            ...
                                            {project.url && (
                                                <a
                                                    ...
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="absolute top-4 right-4 p-2 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-md text-white transition-colors"
                                                    onClick={handlePreviewAction}
                                                >
                                            ...
                                            <div className="space-y-1">
                                                <h3 className="text-xl md:text-2xl font-bold leading-tight drop-shadow-sm pr-2">{project.title}</h3>
                                                {project.description && (
                                                    <p className="text-sm md:text-base text-white/80 line-clamp-2 leading-relaxed max-w-[95%]">
                                                        {project.description}
                                                    </p>
                                                )}
                                            </div>
                                    ...
                                    <div className="relative flex flex-col gap-3">
                                        {project.url && (
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="absolute -top-2 -right-2 p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors"
                                                onClick={handlePreviewAction}
                                            >
                                                <ExternalLink className="w-4 h-4 opacity-50 hover:opacity-100" />
                                            </a>
                                        )}
                                        {hasTags && (
                                            <div className="flex flex-wrap gap-1.5 pr-8">
                                                {project.tags?.map((tag, tIdx) => (
                                                    <span key={tIdx} className="px-2 py-0.5 rounded bg-black/5 dark:bg-white/10 text-[9px] font-bold uppercase tracking-wider opacity-70">
                                                        {tag}
                                                    </span>
                                                ))}
                                            </div>
                                        )}
                                        <div className={!hasTags ? "mt-1" : ""}>
